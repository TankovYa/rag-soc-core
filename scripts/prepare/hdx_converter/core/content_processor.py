...
class ContentProcessor:
    def __init__(self, config, path_resolver, image_processor: ImageProcessor, resolve_link_callback=None):
        ...
        Args:
            ...
            resolve_link_callback: callback для разрешения ссылок (ДОБАВЛЕНО)
        """
        self.config = config
        self.html_parser = HTMLParser()
        self.link_processor = LinkProcessor(path_resolver)
        self.image_processor = image_processor
        self.resolve_link_callback = resolve_link_callback  # ДОБАВЛЕНО
    ...
    def extract_content_with_links(self, soup: BeautifulSoup, source_file: Path) -> Tuple[Dict[str, Any], List, List]:
        ...
        if navigation_text:
            # === ИСПРАВЛЕНИЕ: Добавляем навигацию как текстовый элемент ===
            structured_data["content"].append({
                "type": "navigation",
                "content": navigation_text
            })
            logger.debug(f"Добавлена навигация: {navigation_text[:100]}...")
            # === КОНЕЦ ИСПРАВЛЕНИЯ ===
        ...
        logger.debug(f"Sections: {len(structured_data['content'])}, "
        ...
        logger.debug(f"Created legacy links: internal={len(internal_links_legacy)}, external={len(external_links_legacy)}")
    ...
    def _process_image(self, element: Tag, source_file: Path, context: str) -> Optional[Dict]:
        ...
        if image_path:
            ...
            # Нужно получить доступ к stats_collector
            # Поскольку у нас нет прямого доступа, это нужно делать на уровне converter
            # Вместо этого, вернем специальный флаг в результате
            # === КОНЕЦ ИСПРАВЛЕНИЯ ===

            # === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ 4: Правильный путь к изображениям ===
    ...
    def _extract_link_data(self, element: Tag, source_file: Path, context: str) -> Optional[Dict]:
        ...
        # === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ 1: Для внутренних ссылок используем resolve_link_callback ===
    ...
    def _extract_cli_command_simple(self, pre_element) -> str:
        ...
        logger.debug(f"Извлечение CLI команды (сохранены переносы строк):")
        logger.debug(f"  Исходный текст: '{all_text[:200]}...'")
        logger.debug(f"  Очищенный текст (первые 200 символов): '{cleaned_text[:200]}...'")
    ...
    def _process_ullinks_list(self, element: Tag, source_file: Path, context: str) -> Dict:
        ...
        for link in element.find_all('a', recursive=True):
            link_data = self._extract_link_data(link, source_file, context)
            if link_data and link_data["type"] in ["internal", "external"]:
                # === ИСПРАВЛЕНИЕ ПРОБЛЕМЫ 1: Используем обновленный href из link_data ===
                list_items.append({
                    "type": "list_item",
                    "content": [{
                        "type": "link",
                        "text": link_data["text"],
                        "href": link_data["href"],  # Уже содержит правильное .md имя
                        "link_type": link_data["type"]
                    }]
                })
    ...
    def _collect_links_from_structured_data(self, structured_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger = logging.getLogger('HDXConverter')
        logger.debug(f"=== _collect_links_from_structured_data НАЧАЛО ===")
        ...
        def collect_from_element(element, depth=0):
            ...
            if isinstance(element, dict):
                ...
                if element.get("type") == "link":
                    ...
                    logger.debug(f"{indent}Найдена ссылка: text='{link_info['text'][:30]}...', href='{link_info['href']}', type='{link_info['link_type']}'")
                    ...
                    if element.get("link_type") == "internal":
                        if link_info not in structured_data["links"]["internal"]:
                            ...
                            logger.debug(f"{indent}  -> Добавлена во internal_links")
                        else:
                            logger.debug(f"{indent}  -> Уже есть во internal_links, пропускаем")
                        ...
                        if link_info not in structured_data["links"]["external"]:
                            ...
                            logger.debug(f"{indent}  -> Добавлена во external_links")
                        else:
                            logger.debug(f"{indent}  -> Уже есть во external_links, пропускаем")
                    else:
                        logger.debug(f"{indent}  -> Неизвестный тип ссылки: {element.get('link_type')}")

                # Рекурсивно обрабатываем вложенные элементы
                for key, value in element.items():
                    if key != "type":  # Пропускаем type чтобы избежать бесконечной рекурсии
                        logger.debug(f"{indent}Рекурсивный обход ключа: {key}, тип значения: {type(value)}")
                        collect_from_element(value, depth + 1)

            elif isinstance(element, list):
                logger.debug(f"{indent}Найден список: {len(element)} элементов")
                for i, item in enumerate(element):
                    logger.debug(f"{indent}  Элемент {i+1}/{len(element)}")
                    collect_from_element(item, depth + 1)

        # Собираем ссылки из всего контента
        logger.debug(f"Начало сбора ссылок из structured_data['content']")
        content_items = structured_data.get("content", [])
        logger.debug(f"Количество элементов в content: {len(content_items)}")

        for i, content_item in enumerate(content_items):
            logger.debug(f"Обработка content_item {i+1}/{len(content_items)}")
            collect_from_element(content_item)

        logger.debug(f"=== _collect_links_from_structured_data КОНЕЦ ===")
        logger.debug(f"  Найдено internal_links: {len(structured_data['links']['internal'])}")
        logger.debug(f"  Найдено external_links: {len(structured_data['links']['external'])}")

        return structured_data
    
    def _process_footer_navigation(self, soup: BeautifulSoup, source_file: Path) -> Optional[str]:
        """Обработка навигации в footer - ИСПРАВЛЕННАЯ ВЕРСИЯ
        ...
        if parentlink_div:
            ...
            if parent_link:
                ...
                if href and href.lower().endswith('.html'):
                    # === ИСПРАВЛЕНИЕ: Используем callback для разрешения ссылки ===
    ...
    def _process_task_section(self, section_div: Tag, source_file: Path) -> Optional[Dict]:
        ...
        if header:
            ...
            # УДАЛЯЕМ заголовок из DOM, чтобы он не обрабатывался как текст
            header.decompose()
        else:
        ...
        # Обрабатываем всё содержимое секции (заголовок уже удален)
